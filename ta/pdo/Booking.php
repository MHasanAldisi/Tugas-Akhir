...
class Booking{
    ...
    public function read_booking_2($tgl){
        try{
            
            $sql = "select * from tb_detail_booking, tb_item, tb_boking where
            tb_detail_booking.id_item=tb_item.id_item and tb_detail_booking.id_booking=tb_boking.id_boking
            and tb_boking.status_aktif='2' and tb_boking.tanggal like '%$tgl%' order by tb_boking.tanggal asc";
    ...
    public function read_booking_admin(){
        try{
            
            $sql = "select * from tb_detail_booking, tb_item, tb_boking where
            tb_detail_booking.id_item=tb_item.id_item and tb_detail_booking.id_booking=tb_boking.id_boking
            and tb_boking.status_aktif='2' order by tb_boking.tanggal asc";
            // tb_detail_booking, tb_item where tb_detail_booking.id_item=tb_item.id_item and 
            $stmt = $this->db->prepare($sql);
            $stmt->execute();
            return $stmt;
    ...
    public function read_booking_admin_2(){
        try{
            
            $sql = "select * from tb_detail_booking, tb_item, tb_boking where
            tb_detail_booking.id_item=tb_item.id_item and tb_detail_booking.id_booking=tb_boking.id_boking
            and tb_boking.status_aktif='0' order by tb_boking.tanggal asc";
            // tb_detail_booking, tb_item where tb_detail_booking.id_item=tb_item.id_item and 
            $stmt = $this->db->prepare($sql);
            $stmt->execute();
            return $stmt;
    ...
    public function update_status($id_boking){
     try{
            $sql = "UPDATE tb_boking SET status_aktif = '2' WHERE id_boking = '$id_boking'";
            $stmt = $this->db->prepare($sql);
            $stmt->execute();
            return $stmt;
        }catch(PDOException $e){
    ...
    public function read_item($id_member, $tgl){
        try{
            ...
            (SELECT tb_detail_booking.id_item from tb_boking, tb_detail_booking 
            where tb_boking.id_boking=tb_detail_booking.id_booking and tb_boking.tanggal = '$tgl')
            and tb_item.id_item NOT IN (SELECT id_item FROM tb_keranjang WHERE id_member=$id_member)
            and tb_item.id_item NOT IN (SELECT tb_detail_booking.id_item from tb_detail_booking, tb_boking 
            WHERE tb_boking.id_boking=tb_detail_booking.id_booking and tb_boking.id_member=$id_member 
            and tb_boking.tanggal='$tgl')
            ORDER BY tb_item.nama_pelayanan ASC";
            $stmt = $this->db->prepare($sql);
            // print_r($sql);
            $stmt->execute();
            return $stmt;
    ...
    public function create($id_boking, $id_member, $nama_lengkap, $alamat, $no_hp, $email, $keterangan_item, $tgl){
        try{
            $sql = "INSERT into tb_boking values($id_boking, '$tgl', '$id_member', '$nama_lengkap', '$alamat', '$no_hp', '$email', '$keterangan_item', 1)";
            $stmt = $this->db->prepare($sql);
            $stmt->execute();
            return $stmt;
        }catch(PDOException $e){
